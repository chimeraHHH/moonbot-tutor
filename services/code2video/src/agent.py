...
# Improve Windows charset compatibility for Chinese logs and multiprocessing workers.
...
# Ensure project-root imports (e.g., prompts/) work when running from src/.
...
class TeachingVideoAgent:
    def __init__(
        ...
        self.manim_quality_flag = cfg.manim_quality_flag
        self.manim_quality_dir = cfg.manim_quality_dir
        ...
        if (repo_root / "assets").exists() and (repo_root / "json_files").exists():
            self.resource_root = repo_root
        else:
            self.resource_root = Path(*self.output_dir.parts[: self.output_dir.parts.index("CASES")])

        self.assets_dir = self.resource_root / "assets" / "icon"
        ...
        """4. External Database"""
        knowledge_ref_mapping_path = self.resource_root / "json_files" / "long_video_ref_mapping.json"
        with open(knowledge_ref_mapping_path) as f:
            self.KNOWLEDGE2PATH = json.load(f)
        self.knowledge_ref_img_folder = self.resource_root / "assets" / "reference"
        ...
        for fix_attempt in range(max_fix_attempts):
            ...
            try:
                ...
                if result.returncode == 0:
                    video_patterns = [
                        self.output_dir / "media" / "videos" / f"{code_file.replace('.py', '')}" / self.manim_quality_dir / f"{scene_name}.mp4",
                        self.output_dir / "media" / "videos" / self.manim_quality_dir / f"{scene_name}.mp4",
                    ]
                ...
                fixed_code = self.scope_refine_fixer.fix_code_smart(section_id, current_code, error_output, self.output_dir)
    ...
    def merge_videos(self, output_filename: str = None) -> str:
        ...
        with open(video_list_file, "w", encoding="utf-8") as f:
            for section_id in sorted(self.section_videos.keys()):
                ...
                video_path = Path(self.section_videos[section_id])
                relative_path = video_path.relative_to(self.output_dir)
                # Convert to forward slashes for WSL
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"❌ Failed to merge section videos: {error_msg}")
                return None
        except Exception as e:
...
if __name__ == "__main__":
    ...
    if _CFG_PATH.exists():
        ...
        print(f"WARNING: Config file not found: {_CFG_PATH}.")
        _CFG = {}
