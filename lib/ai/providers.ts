...
function normalizeMiniMaxAnthropicBaseUrl(
  ...
  // @ai-sdk/anthropic appends /messages to baseURL, so the URL must include /v1.
  // If the configured base URL doesn't end with /v1, append it so that the SDK
  // constructs the correct path (e.g. https://host/claude → https://host/claude/v1).
...
export function getModel(config: ModelConfig): ModelWithInfo {
  ...
  switch (providerType) {
    ...
    case 'google': {
      // Vertex mode: activated when GOOGLE_VERTEX_PROJECT is set. Uses
      // @ai-sdk/google-vertex/edge (API-key / Express auth). Otherwise falls
      // back to AI Studio via @ai-sdk/google.
      const vertexProject =
      ...
      // Effective proxy: explicit config.proxy > env HTTPS_PROXY/HTTP_PROXY
      const envProxy =
