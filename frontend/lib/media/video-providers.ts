...
import { generateWithDeepSolve, testDeepSolveConnectivity } from './adapters/deep-solve-adapter';
...
export const VIDEO_PROVIDERS: Record<VideoProviderId, VideoProviderConfig> = {
  ...
  // Deep Solve (Code2Video / Manim) — an external step-by-step explainer-video
  // pipeline. Unlike commercial T2V providers, it turns a *problem/topic* into a
  // rendered Manim video via an async task API (submit → poll → download).
  // Keyless: registered by base URL alone (the bridge endpoint). See
  // deep-solve-adapter.ts and VIDEO_DEEPSOLVE_* in provider-config.ts.
  'deep-solve': {
    id: 'deep-solve',
    name: 'Deep Solve (Code2Video / Manim)',
    requiresApiKey: false,
    defaultBaseUrl: 'http://localhost:8010',
    models: [{ id: 'code2video', name: 'Deep Solve (Manim)' }],
...
export async function testVideoConnectivity(
  ...
  switch (config.providerId) {
    ...
    case 'deep-solve':
      return testDeepSolveConnectivity(config);
    default:
      return {
        success: false,
...
export async function generateVideo(
  ...
  switch (config.providerId) {
    ...
    case 'deep-solve':
      return generateWithDeepSolve(config, options);
    default:
