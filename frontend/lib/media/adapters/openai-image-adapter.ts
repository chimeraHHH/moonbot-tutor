...
export async function testOpenAIImageConnectivity(
  ...
  try {
    const response = await fetch(`${baseUrl}/models/${encodeURIComponent(targetModel)}`, {
    ...
    // Some providers (e.g. custom proxies) don't support single-model lookup.
    // Fall back to listing all models and checking membership.
    if (response.status === 404) {
      ...
      if (listRes.ok) {
        ...
        return {
          success: false,
          message: `OpenAI Image model not found: ${targetModel}`,
        };
      }
    }
