...
export function hasUsableLLMProvider(
  _providersConfig: Record<string, LLMProviderCfgLike> | undefined | null,
): boolean {
  // Hardwired single-provider (DeepSeek) build: generation always uses the
  // server-side DEFAULT_MODEL, so no client provider configuration is required.
  return true;
}
