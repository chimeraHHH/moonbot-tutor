...
export function useChatSessions(options: UseChatSessionsOptions = {}) {
  ...
  const appendPeerMessage = useCallback((agent: ClassroomPeerAgent, text: string) => {
    ...
    const sessionId = `peer-exchange-${currentStageId}`;
    const now = Date.now();
    ...
    setSessions((previous) => {
      ...
      const lessonLocale = useStageStore.getState().stage?.lessonLanguage?.locale || 'zh-CN';
      return [
        ...previous,
        {
          id: sessionId,
          type: 'discussion',
          title: lessonLocale.startsWith('zh') ? '同学交流' : 'Peer exchange',
          status: 'completed',
          messages: [message],
          config: { agentIds: [] },
          toolCalls: [],
